...
export type DispatchResult = {
  ...
  created: boolean;
  channels: { in_app: boolean; web_push: 'queued' | 'skipped' | 'no_endpoints' };
  reason?: string;
};
...
export const dispatchNotification = async (
  ...
  if (!isCategoryAllowed(prefs, input.category)) {
    return {
      notification: null,
      created: false,
      channels: { in_app: false, web_push: 'skipped' },
  ...
  // Push delivery
  let webPushChannelStatus: DispatchResult['channels']['web_push'] = 'skipped';
  if (isChannelAllowed(prefs, 'web_push')) {
  ...
  return {
    notification: row,
    created,
    channels: { in_app: true, web_push: webPushChannelStatus },
  };
};
